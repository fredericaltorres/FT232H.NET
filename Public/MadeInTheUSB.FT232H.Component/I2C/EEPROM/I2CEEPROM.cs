...
namespace MadeInTheUSB.FT232H.Component.I2C.EEPROM
{
    ...
    // DeviceID Chip is wired for address A0=GND A1=GND A2=GND == 0xA0 -- 128 64 32 16 8 4-A2 2-A1 1-A0
    ...
    public class I2CEEPROM_AT24C256
    {
        ...
        public const byte AT24C256_DEVICE_ID = 0xA0; // A0=GND A1 = GND A2=GND == 0xA0 -- 128 64 32 16 8 4-A2 2-A1 1-A0
        ...
        public bool WritePages(int addr, List<byte> buffer, bool verify = false)
        {
            ...
            return this._i2cDevice.WriteBuffer(buffer2.ToArray(), this.DeviceId);
        }
        ...
        public bool ReadPages(int addr, int byteToRead, List<byte> buffer)
        {
            ...
            if(this._i2cDevice.WriteBuffer(buffer2.ToArray(), this.DeviceId, terminateTransmission: false))
            {
                buffer = this._i2cDevice.ReadXByte(byteToRead, this.DeviceId);
                return true;
            }
            else
            {
